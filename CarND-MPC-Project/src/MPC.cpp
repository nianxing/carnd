...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  retValue.push_back(solution.x[delta_start]);
  retValue.push_back(solution.x[a_start]);
  for(int i=0;i<N-1;i++)
  {
    retValue.push_back(solution.x[x_start+i]);
    retValue.push_back(solution.x[y_start+i]);
  }
  return retValue;
}
